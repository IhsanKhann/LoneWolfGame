...
void DecisionTree::loadNodes() {
    ...
    addChoice(1, "Follow the scent of blood", 2);
    addChoice(1, "Wait and howl for pack", 3);
    addTrigger(1, 1);
    ...
    addChoice(2, "Cross the ice carefully", 4);
    addChoice(2, "Find another path around", 5);
    addTrigger(2, 2);
    ...
    addChoice(3, "Hide and observe them", 6);
    addChoice(3, "Approach cautiously", 7);
    addTrigger(3, 3);
    ...
    addChoice(4, "Rush forward to catch deer", 8);
    addChoice(4, "Retreat to safety", 9);
    addTrigger(4, 4);
    ...
    addChoice(5, "Enter the cave cautiously", 10);
    addChoice(5, "Continue hunting elsewhere", 11);
    addTrigger(5, 5);
    ...
    addChoice(6, "Challenge the scarred leader", 12);
    addChoice(6, "Show submission and respect", 13);
    addTrigger(6, 6);
    ...
    addChoice(7, "Tell truth about the storm", 14);
    addChoice(7, "Claim you're a lone wolf", 15);
    addTrigger(7, 7);
    ...
    addChoice(9, "Hunt the rabbit", 16);
    addChoice(9, "Search for better shelter", 17);
    addTrigger(9, 9);
    ...
    addChoice(10, "Memorize the map and take supplies", 18);
    addChoice(10, "Leave respectfully and continue", 19);
    addTrigger(10, 10);
    ...
    addChoice(16, "Defend your kill from bear", 20);
    addChoice(16, "Abandon kill and flee", 21);
    addTrigger(16, 16);
    ...
    addChoice(17, "Stay hidden in shelter", 22);
    addChoice(17, "Investigate the wolf fight", 23);
    addTrigger(17, 17);
    ...
    addChoice(19, "Search for your original pack", 24);
    addChoice(19, "Seek out other lone wolves", 25);
    addTrigger(19, 19);
    ...
    addChoice(21, "Hunt again immediately", 11);
    addChoice(21, "Find safe place to rest", 22);
    addTrigger(21, 21);
    ...
    addChoice(23, "Help the wounded wolf", 24);
    addChoice(23, "Approach the victor", 25);
    addTrigger(23, 23);
...
bool DecisionTree::makeChoice(int choiceIndex) {
    ...
    currentNodeId = choices[choiceIndex].second;
    return true;
}

void DecisionTree::reset() { currentNodeId = 1; }
void DecisionTree::setCurrentNode(int nodeId) { currentNodeId = nodeId; }
void DecisionTree::createNode(int id, const std::string& text, bool isEnding) { nodes[id] = new Node(id, text, isEnding); }
void DecisionTree::addChoice(int nodeId, const std::string& text, int nextNodeId) {
    auto it = nodes.find(nodeId);
    if (it != nodes.end()) it->second->addChoice(text, nextNodeId);
}
...
void DecisionTree::generateDotFile(const std::string& filename) const {
    ...
    for (const auto& pair : nodes) {
        const Node* node = pair.second;
        for (size_t i = 0; i < node->getChoices().size(); ++i)
            file << "  node" << node->getId() << " -> node" << node->getChoices()[i].second << " [label=\"" << static_cast<char>('A' + i) << "\"];\n";
    }
